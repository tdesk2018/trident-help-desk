...
exports.create = function(req, res) {
   ...
   //if(!req.body.content) {
    ...
    //}
    var note = new Note({name: req.body.name || "Untitled Note", password: req.body.password});
...
exports.authenticate = function(req,res){
    ...
    Note.findById(req.body.username, req.body.password, function(err, data) {
        if(err) {
            res.status(500).send({message: "Could not retrieve note with id " + req.params.username});
        } else {
            res.send(data);
        }
    });
